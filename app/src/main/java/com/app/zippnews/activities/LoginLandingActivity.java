...
import com.app.zippnews.servicerequestmodels.SocialInfoRequest;
import com.app.zippnews.servicerequestmodels.SocialloginRequest;
import com.app.zippnews.presenter.SocialLoginPresenter;
...
public class LoginLandingActivity extends AppCompatActivity implements View.OnClickListener,SocialLoginPresenter.SocialloginResponseListeners {
    ...
    private static String TWITTER_CONSUMER_KEY = "H4SY6jAy902C7yHNyoUnutX9O";
    private static String TWITTER_CONSUMER_SECRET = "Bs4an1BDiU486QpwkhIMdPo6x27rPipPf7d7myQ9QIlwDIGji0";
    ...
    private void updateUI(GoogleSignInAccount account)
    {
        if(account!=null) {
            ...
            callSocialSignupService("google",""+id,email,account.getDisplayName());
        }
    }
        ...
        TwitterCore.getInstance().getApiClient(session).getAccountService().verifyCredentials(true, false, true).enqueue(new retrofit2.Callback<User>() {
            ...
            public void onResponse(Call<User> call, Response<User> response) {
                if (response.isSuccessful()){
                    ...
                    Log.e("name",""+username);
                    callSocialSignupService("twitter",""+user.getId(),usermail,username);
                }
            }
        ...
        final GraphRequest request = GraphRequest.newMeRequest(accessToken, new GraphRequest.GraphJSONObjectCallback() {
            ...
            public void onCompleted(JSONObject object, GraphResponse response) {
                if (response.getError() != null) {
                    ...
                    callSocialSignupService("facebook",response.getRequest().getAccessToken().getUserId(),object.optString("email"),object.optString("first_name"));
                    LoginManager.getInstance().logOut();
                }
            }
        });
    ...
    private void callSocialSignupService(String type,String socialid,String email,String name)
    {
        SocialloginRequest request=new SocialloginRequest();
        SocialInfoRequest socialInfoRequest=new SocialInfoRequest();
        socialInfoRequest.setEmail(email);
        socialInfoRequest.setUsername(name);
            request.setType(type);
            request.setUsername(socialid);
            request.setInfo(socialInfoRequest);
